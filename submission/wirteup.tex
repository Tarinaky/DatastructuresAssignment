...
\begin{program}
\BEGIN
cursor := start \rcomment{$start$ is the first word in the ladder}
|push | cursor | on | stack
\FOR i := 1 \TO depth \DO \rcomment{$depth$ is the length of the ladder}
	edges := | get | edges | from | cursor \rcomment{Expand edges}
	\FOR seen | in | stack \DO
		|remove | seen | from | edges  \rcomment{Prune visited elements}
	\OD
	\IF edges | is empty | \DO 
		i := i - 2 \rcomment{Backtrack}
		cursor := |pop from | stack 
	\ELSE
		cursor := |random from | edges \rcomment{Take a random walk}
		|push | cursor | on | stack 
	\OD
\OD
\IF stack | length equals | depth | |\DO
	|return | stack 
\ELSE
